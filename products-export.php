...
function marfoof_connect_export_products(WP_REST_Request $request) {
    ...
    switch ($action) {
        case 'preview':
            return marfoof_connect_preview_products();
        ...
        case 'start_migration':
            ...
            return marfoof_connect_start_migration('products', $batch_size, $force_restart);
            
        case 'migrate_batch':
            $batch_number = isset($params['batch']) ? (int)$params['batch'] : 1;
            $batch_size = isset($params['batch_size']) ? (int)$params['batch_size'] : 500;
            return marfoof_connect_migrate_batch('products', $batch_number, $batch_size);
            
        case 'migration_status':
            return marfoof_connect_get_migration_status('products');
            
        case 'reset_migration':
            return marfoof_connect_reset_migration('products');
            
        case 'get_all':
            return marfoof_connect_get_all_products();
 ...
 */
function marfoof_connect_preview_products() {
    $stats = marfoof_connect_get_migration_stats('products');
 ...
 */
function marfoof_connect_start_migration($type, $batch_size = 500, $force_restart = false) {
    global $wpdb;
    
    if ($force_restart) {
        // Clear existing tracking data
        $wpdb->delete($wpdb->prefix . 'marfoof_migration_tracking', 
            array('migration_type' => $type)
        );
    }
    
    // Get total count for planning
    $stats = marfoof_connect_get_migration_stats($type);
 ...
 */
function marfoof_connect_migrate_batch($type, $batch_number = 1, $batch_size = 500) {
    global $wpdb;
    
    // Get the offset (how many items already migrated)
    $offset = marfoof_connect_get_migration_offset($type);
    ...
    if ($type === 'products') {
        ...
        $items = $wpdb->get_results(
            $wpdb->prepare(
                ...
                LEFT JOIN {$wpdb->prefix}marfoof_migration_tracking mt 
        ...
        foreach ($items as $item) {
            try {
                $item_data = marfoof_connect_prepare_product_data($item);
                if ($item_data) {
                    $items_data[] = $item_data;
                    
                    // Mark as migrated
                    marfoof_connect_mark_item_migrated($type, $item->ID, $batch_number);
                }
            } catch (Exception $e) {
                marfoof_connect_mark_item_migrated($type, $item->ID, $batch_number, $e->getMessage());
            }
        }
        ...
        foreach ($items as $item) {
            try {
                $item_data = marfoof_connect_prepare_user_data($item);
                if ($item_data) {
                    $items_data[] = $item_data;
        ...
        foreach ($items as $item) {
            try {
                $item_data = marfoof_connect_prepare_category_data($item);
                if ($item_data) {
                    $items_data[] = $item_data;
                    
                    // Mark as migrated
                    marfoof_connect_mark_item_migrated($type, $item->term_id, $batch_number);
                }
            } catch (Exception $e) {
                marfoof_connect_mark_item_migrated($type, $item->term_id, $batch_number, $e->getMessage());
            }
        }
    }
 ...
 */
function marfoof_connect_get_migration_status($type = 'products') {
    $stats = marfoof_connect_get_migration_stats($type);
 ...
 */
function marfoof_connect_reset_migration($type = 'products') {
    global $wpdb;
    
    $table_name = $wpdb->prefix . 'marfoof_migration_tracking';
 ...
 */
function marfoof_connect_get_all_products() {
    global $wpdb;
    ...
    foreach ($products as $product) {
        $product_data = marfoof_connect_prepare_product_data($product);
        if ($product_data) {
 ...
 */
function marfoof_connect_prepare_product_data($product_post) {
    global $wpdb;
 ...
 */
function marfoof_connect_prepare_user_data($user) {
 ...
 */
function marfoof_connect_prepare_category_data($category) {
    // Get category image
