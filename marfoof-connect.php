...
register_activation_hook(__FILE__, 'marfoof_connect_create_tracking_table');
...
add_action('plugins_loaded', function() {
    ...
    if($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) {
        marfoof_connect_create_tracking_table();
    }
});
...
function marfoof_connect_register_api_route()
{
    ...
    // Users endpoints
    register_rest_route("marfoof-connect/v1", "/export/users", [
        "methods" => "GET",
    ...
    register_rest_route("marfoof-connect/v1", "/test-server", [
        "methods" => "GET",
        "callback" => "marfoof_connect_test_server",
        "permission_callback" => "marfoof_connect_check_api_key",
    ]);
}
    ...
    <div class="wrap">
        ...
        <script>
            ...
            function testServerConnection() {
                ...
                // Test server endpoint
                fetch(`<?php echo rest_url("marfoof-connect/v1/test-server"); ?>?server_url=${encodeURIComponent(serverUrl)}`, {
                    headers: {
            ...
            function loadMigrationPreview(type) {
                ...
                fetch(`<?php echo rest_url("marfoof-connect/v1/export/"); ?>${type}?action=preview`, {
                    headers: {
                        'Authorization': 'Bearer <?php echo MARFOOF_CONNECT_API_KEY; ?>'
                    }
                })
            ...
            function startMigration(type) {
                ...
                fetch(`<?php echo rest_url("marfoof-connect/v1/export/"); ?>${type}?action=start_migration&batch_size=${batchSize}&force_restart=${forceRestart}`, {
                    headers: {
                        'Authorization': 'Bearer <?php echo MARFOOF_CONNECT_API_KEY; ?>'
                    }
                })
            ...
            function processNextBatch(type, batchNumber, batchSize) {
                ...
                fetch(`<?php echo rest_url("marfoof-connect/v1/export/"); ?>${type}?action=migrate_batch&batch=${batchNumber}&batch_size=${batchSize}`, {
                    headers: {
                        'Authorization': 'Bearer <?php echo MARFOOF_CONNECT_API_KEY; ?>'
                    }
                })
                ...
                try {
                    const response = await fetch(`${serverUrl}/import/${type}`, {
                        ...
                        headers: {
                            ...
                            'X-API-Key': 'marfoof-secret-static-key-here-123'
                        },
            ...
            function downloadAllData(type) {
                ...
                fetch(`<?php echo rest_url("marfoof-connect/v1/export/"); ?>${type}?action=get_all`, {
                    headers: {
                        'Authorization': 'Bearer <?php echo MARFOOF_CONNECT_API_KEY; ?>'
                    }
                })
            ...
            function checkMigrationStatus(type) {
                fetch(`<?php echo rest_url("marfoof-connect/v1/export/"); ?>${type}?action=migration_status`, {
                    headers: {
                        'Authorization': 'Bearer <?php echo MARFOOF_CONNECT_API_KEY; ?>'
                    }
                })
            ...
            function resetMigration(type) {
                ...
                fetch(`<?php echo rest_url("marfoof-connect/v1/export/"); ?>${type}?action=reset_migration`, {
                    headers: {
                        'Authorization': 'Bearer <?php echo MARFOOF_CONNECT_API_KEY; ?>'
                    }
                })
...
// Test server connection
function marfoof_connect_test_server(WP_REST_Request $request) {
